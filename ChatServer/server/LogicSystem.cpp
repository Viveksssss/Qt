...
void LogicSystem::RegisterCallBacks() {
  ...
  _function_callbacks[MsgId::ID_CHAT_LOGIN] = [this](std::shared_ptr<Session>
    ...
    bool b_apply =
        MysqlManager::GetInstance()->GetFriendApplyList(uid_str, apply_list);
    ...
    if (b_notify && notification_list.size() > 0) {
      ...
      for (auto &notification : notification_list) {
        ...
        item["type"] =
            notification->status; // 用status代表type借用UserInfo的结构。
        item["message"] =
...
bool LogicSystem::GetBaseInfo(std::string base_key, int uid,
  ...
  if (b_base) {
    ...
    SPDLOG_INFO("uid:{},name:{},email:{}", uid, userinfo->name,
                userinfo->email);
  } else {
...
LogicSystem::LogicSystem(std::size_t size) : _stop(false), _size(size) {
  RegisterCallBacks();
