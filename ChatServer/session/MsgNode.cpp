...
SendNode::SendNode(const char* msg, uint16_t max_len, uint16_t msg_id)
    ...
    , _msg_id(msg_id)
{
    uint16_t msg_id_host = boost::asio::detail::socket_ops::host_to_network_long(msg_id);
    memcpy(_data, &msg_id_host, HEAD_ID_LEN);

    uint16_t max_len_host = boost::asio::detail::socket_ops::host_to_network_long(max_len);
