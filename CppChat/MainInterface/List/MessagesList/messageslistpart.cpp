    ...
    , dataReady(false)  // 添加数据就绪标志
{
    ...
    QTimer::singleShot(200, this, [this]() {
        if (!UserManager::GetInstance()->GetMessages().empty()) {
            qDebug() << "Initial messages available, loading...";
            do_loading_messages();
        } else {
            qDebug() << "No messages available initially, waiting for network data...";
            // 如果没有数据，设置一个重试机制
            QTimer::singleShot(1000, this, [this]() {
                if (!dataReady && !UserManager::GetInstance()->GetMessages().empty()) {
                    qDebug() << "Retrying message loading after delay...";
                    do_loading_messages();
                }
            });
        }
...
void MessagesListPart::do_loading_messages()
{
    ...
    isLoading = true;
    qDebug() << "Loading messages, count:" << messages.size();
    ...
    QTimer::singleShot(1000, this, [this](){
        this->setLoading(false);
        qDebug() << "Messages loading completed";
    });
}

void MessagesListPart::do_add_message_to_list(const ConversationItem &info)
{
    qDebug() << "Adding single message to list";
    messagesModel->addMessage(info);
...
void MessagesListPart::do_add_messages_to_list(const std::span<std::shared_ptr<ConversationItem>> &list)
{
    qDebug() << "Adding" << list.size() << "messages to list";

    if (list.empty()) {
        qDebug() << "Empty message list received";
        return;
    }
    ...
    qDebug() << "Messages added to list successfully";
}
